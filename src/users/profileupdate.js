...
function Profileupdate() {
  ...
  const chatsquery = firestore.collection("chats").where('uid','!=',user.currentUser.uid).where('chatparticipants','array-contains',user.currentUser.uid);
 ...
 try{
    ...
    value.forEach(async v=>{
      ...
      if(v.chatparticipants.length===2){
        console.log(v.chatparticipants[0] + v.chatparticipants[1])
      await  chatsRef.doc(v.chatparticipants[0] + v.chatparticipants[1]).update({
          imageUrl : url
        })
      }
    })
  ...
  return (
    ...
    >
      ...
      <Imageupload Upload={{file,setfile,submitHandler,setpreviewUrl,previewUrl,loading,btn:true}}/>
      </section>
    </main>
  );
}
